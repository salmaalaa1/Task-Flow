...
class FirestoreDatabaseService {
  ...
  String _hashJoinPassword(String password) =>
  ...
  Future<FirestoreTeam> createTeam({
    required String name,
    ...
    final ownerMember = TeamMemberRecord(
      userId: ownerUserId,
      name: ownerName,
      email: ownerEmail,
      department: '',
    ...
    batch.set(teamRef.collection('members').doc(ownerUserId), {
      ...ownerMember.toMap(),
  ...
  Future<TeamJoinResponse> joinTeam({
    ...
    final teamQuery = await _teams
        .where('normalizedName', isEqualTo: normalizeTeamName(teamName))
        .limit(1)
        .get();
    if (teamQuery.docs.isEmpty) {
      return const TeamJoinResponse(
        success: false,
        message: 'No team exists with that name.',
      );
    }
    ...
    if (teamData['joinPasswordHash'] != _hashJoinPassword(password)) {
      return const TeamJoinResponse(
        success: false,
        message: 'The team password is incorrect.',
      );
    }
    if (teamData['ownerUserId'] == userId) {
      return const TeamJoinResponse(
        success: false,
        message: 'The team owner cannot join as a member.',
      );
    }

    final memberRef = teamDoc.reference.collection('members').doc(userId);
    final member = TeamMemberRecord(
      userId: userId,
    ...
    try {
      ...
      // The member document is the source of truth; this denormalized list is best-effort.
    }
    ...
    return TeamJoinResponse(
      ...
      team: FirestoreTeam(
        id: teamDoc.id,
        name: teamData['name'] as String? ?? teamName,
        description: teamData['description'] as String? ?? '',
        ownerUserId: teamData['ownerUserId'] as String? ?? '',
        members: [member],
      ),
      member: member,
    );
  ...
  Future<List<TeamMemberRecord>> getTeamMembers(String teamId) async {
    ...
    return snapshot.docs
        .map((doc) => TeamMemberRecord.fromMap(doc.data()))
        .toList(growable: false);
  }

  Stream<List<TeamMemberRecord>> watchTeamMembers(String teamId) {
    return _teams
        .doc(teamId)
        .collection('members')
        .orderBy('joinedAt')
  ...
  Future<void> leaveTeam({required String teamId, required String userId}) {
    return _teams.doc(teamId).collection('members').doc(userId).delete();
  }

  Future<void> deleteTeam({required String teamId}) async {
    final teamRef = _teams.doc(teamId);
    final teamDoc = await teamRef.get();
    final ownerUserId = teamDoc.data()?['ownerUserId'] as String?;
    final tasks = await teamRef.collection('tasks').get();
    final members = await teamRef.collection('members').get();
    final ownerMemberDocs = members.docs
        .where((member) => member.id == ownerUserId)
        .toList();
    final otherMemberDocs = members.docs
        .where((member) => member.id != ownerUserId)
        .toList();
    ...
    Future<void> queueDelete(DocumentReference<Object?> reference) async {
    ...
    for (final task in tasks.docs) {
      await queueDelete(task.reference);
    }
    for (final member in otherMemberDocs) {
      await queueDelete(member.reference);
    }
    if (opCount > 0) {
      await batch.commit();
      batch = _db.batch();
    ...
    opCount++;
    for (final member in ownerMemberDocs) {
  ...
  Stream<List<TeamTask>> watchTeamTasks(String teamId) {
    return _teams
        ...
        .snapshots()
        .map(
          (snapshot) => snapshot.docs.map(_taskFromDoc).toList(growable: false),
        );
  }

  Stream<List<TeamTask>> watchAssignedTasks({
  ...
  Future<bool> updateAssignedTaskStatus({
    required String teamId,
  ...
  Future<bool> updateAssignedTaskNote({
    required String teamId,
    required String taskId,
    required String userId,
